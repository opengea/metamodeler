...
<body>
    ...
    <script>
        ...
        const torusPositions = [
            { x: 0, y: 2, z: 0 }, // TEO: Pol nord
            { x: 0, y: 0, z: 2 }, // FEN: Davant
            { x: 2, y: 0, z: 0 }, // SUB: Costat
            { x: 1.5, y: 0, z: 1.5 }, // ART: Entre costat i davant
            { x: 0, y: 1.5, z: 1.5 }, // ANA: Entre pol nord i davant
            { x: 1.5, y: 1.5, z: 0 }, // STT: Entre pol nord i costat
            { x: 1.2, y: 1.2, z: 1.2 }, // EST: Entre A, N i I
            { x: -1.5, y: 1.5, z: 0 }, //  SGT
            { x: 0, y: 1.5, z: -1.5 }, // SIN
	    { x: 1.2, y: 1.2, z: -1.2 }, // MIT
	    { x: 1.5, y: 0, z: -1.5 }, // MTP
	    { x: -1.2, y: 1.5, z: -1.2 }, // IDE
	    { x: -1.2, y: 1.5, z: 1.2 } // LOG
        ];
        ...
        function createTorusScene() {
            ...
            for (let i = 0; i < torusPositions.length; i++) {
                const hue = i / torusPositions.length;
                const color = hslToRgb(hue, 1, 0.5);

                const torus = createTorusWireframe(radius, tube, radialSegments, tubularSegments, color);
                torus.position.set(torusPositions[i].x, torusPositions[i].y, torusPositions[i].z);

                // Orientació del forat del torus cap al centre de l'escena
                const lookAtPos = new THREE.Vector3(0, 0, 0);
                torus.lookAt(lookAtPos);
